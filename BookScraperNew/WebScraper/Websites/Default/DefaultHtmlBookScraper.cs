...
namespace WebScraper.Websites.Default
{
    class DefaultHtmlBookScraper: HtmlBookScraper
    {
        ...
        */
        ...
        public DefaultHtmlBookScraper()
        {
            //OriginalDoc = new HtmlDocument();
        }

        public DefaultHtmlBookScraper(string oDoc)
        {
            OriginalDoc = new HtmlDocument();
                    ...
                    if (node.Attributes != null)
                    {
                        if (node.Attributes["class"] != null)
                        {
                                ...
                                if (node.Attributes["class"].Value.Equals("entry-content"))
                            {
                                ...
                                if (node.Attributes["class"].Value.Equals("entry-content content"))
                            {
                                if (!(node.ChildNodes.Count < 10))
                                {
                                    this.ReducedDoc = node;
            ...
            foreach (HtmlNode node in doc.ChildNodes)
            {
                if (node.Name.Equals("a"))
                {
                    ...
                    if (node.InnerText.Trim().Equals("Previous Chapter") || node.InnerText.Trim().Equals("Previous")
                        ...
                        || node.InnerText.Trim().Equals("&lt;&lt;Previous Chapter") || node.InnerText.Trim().Equals("&lt;&lt;Previous Chapter", StringComparison.OrdinalIgnoreCase) 
                        || node.InnerText.Trim().Equals("&lt;- Previous Part")
                        || node.InnerText.Trim().Equals("[Previous Chapter]") || node.InnerText.Trim().Equals("<<Previous Chapter"))
                    {
                    ...
                    if (node.InnerText.Trim().Equals("Next Chapter") || node.InnerText.Trim().Equals("Next")
                        ...
                        || node.InnerText.Trim().Equals("Next Chapter&gt;&gt;") || node.InnerText.Trim().Equals("Next Part -&gt;")
                        || node.InnerText.Trim().Equals("NEXT PART&gt;&gt;", StringComparison.OrdinalIgnoreCase) || node.InnerText.Trim().Equals("&gt;&gt;NEXT PART&gt;&gt;", StringComparison.OrdinalIgnoreCase) || node.InnerText.Trim().Equals("[Next Chapter]")
